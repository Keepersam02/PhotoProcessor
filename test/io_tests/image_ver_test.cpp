...
std::vector<image_io_test_case> test_cases2{
    {"tiff_non_raw",
     TIFF_FILE,
     false,
     false,
...
TEST(image_io, image_format) {
  fs::path temp_dir = fs::temp_directory_path();
  for (image_io_test_case t_case : test_cases2) {
    std::ofstream file(temp_dir / t_case.file_name, std::ios::binary);
    file.write(reinterpret_cast<const char *>(t_case.file_header.data()),
               t_case.file_header.size());
    file.close();
  ...
  for (image_io_test_case t_case : test_cases2) {
    auto i_type = image_format(fs::path(temp_dir / t_case.file_name).string());
    EXPECT_EQ(!i_type.has_value(), t_case.ret_err);
    ASSERT_EQ(i_type.value(), t_case.expected_type);
  }
}
